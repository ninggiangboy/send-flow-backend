...
func NewTrackingProviderEventConsumer(svc *trackingapp.Service, log *slog.Logger, brokers []string, groupID string, pool *pgxpool.Pool) *TrackingProviderEventConsumer {
...
func (c *TrackingProviderEventConsumer) Name() string {
	return c.name
}

func (c *TrackingProviderEventConsumer) Run(ctx context.Context) error {
	if len(c.brokers) == 0 {
	...
	return consumer.Consume(ctx, func(ctx context.Context, msg kafka.Message) error {
		...
		if err := c.HandleEvent(ctx, eventID, msg.Value); err != nil {
			...
			c.log.Error("retryable error handling tracking provider event",
				"event_id", eventID,
				"error", err,
			)
			return err
...
func (c *TrackingProviderEventConsumer) HandleEvent(ctx context.Context, eventID string, rawPayload []byte) error {
	...
	if result.Ignored {
		c.log.Info("tracking provider event ignored by type",
			"event_id", envelope.EventID,
			"event_type", payload.EventType,
		)
	} else {
		c.log.Info("tracking provider event handled successfully",
			"event_id", envelope.EventID,
