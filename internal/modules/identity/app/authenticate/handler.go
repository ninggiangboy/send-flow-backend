...
func New(tokens ports.TokenManager, sessionsRead ports.SessionReadRepository, logger *slog.Logger) *Handler {
	return &Handler{tokens: tokens, sessionsRead: sessionsRead, log: logger.With("usecase", "authenticate")}
}

func (h *Handler) Execute(ctx context.Context, token string, now time.Time) (*domain.Session, *domain.User, error) {
