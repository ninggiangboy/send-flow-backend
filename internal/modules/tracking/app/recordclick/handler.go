	...
	if err := h.txManager.WithinTx(ctx, func(txCtx context.Context) error {
		if input.SourceEventID != "" {
			...
			if existing != nil {
				...
				trackingEventResult = &Result{TrackingEventID: existing.ID, DestinationURL: destinationURL}
				return nil
			}
		}
		...
		trackingEventResult = &Result{TrackingEventID: eventID, DestinationURL: destinationURL}
		return nil
	}); err != nil {
