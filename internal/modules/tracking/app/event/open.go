...
func NewOpen(opts OpenOptions) *OpenHandler {
	if opts.Logger == nil {
	...
	return &OpenHandler{
		...
		idGen:           opts.IDGen,
		log:             opts.Logger.With("usecase", "record_open"),
	}
}
...
func (h *OpenHandler) Execute(ctx context.Context, input OpenInput) (*OpenResult, error) {
	...
	var trackingEventResult *OpenResult

	if err := h.txManager.WithinTx(ctx, func(txCtx context.Context) error {
		if input.SourceEventID != "" {
			existing, err := h.eventReadRepo.FindBySourceEvent(txCtx, source, input.SourceEventID, domain.EventTypeOpen)
			if err != nil {
			...
			if existing != nil {
				...
				trackingEventResult = &OpenResult{TrackingEventID: existing.ID}
				return nil
			}
		}
		...
		trackingEventResult = &OpenResult{TrackingEventID: eventID}
		return nil
	}); err != nil {
