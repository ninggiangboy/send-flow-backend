...
func newTestService(
	...
	return NewService(Options{
		FactRepo:                factRepo,
		WorkspaceQueryRepo:      workspaceQueryRepo,
		CampaignQueryRepo:       campaignQueryRepo,
		DeliverabilityQueryRepo: deliverabilityQueryRepo,
...
func TestIngestEmailEventFact_CreateError(t *testing.T) {
	createErr := errors.New("clickhouse write failed")
	svc := newTestService(
		&stubFactRepo{
			FindBySourceEventIDFunc: func(_ context.Context, _ string) (*domain.EmailEventFact, error) {
				return nil, domain.ErrAnalyticsProjectionNotFound
			},
			CreateFunc: func(_ context.Context, _ domain.EmailEventFact) error { return createErr },
		},
		nil, nil, nil, nil,
	)
	err := svc.IngestEmailEventFact(context.Background(), IngestEmailEventFactInput{
...
func TestGetCampaignAnalytics_Pending(t *testing.T) {
	svc := newTestService(
		...
		&stubCampaignQueryRepo{
			GetCampaignFunnelFunc: func(_ context.Context, _, _ string, _, _ time.Time) (*domain.CampaignFunnel, error) {
				return &domain.CampaignFunnel{Status: "pending"}, nil
			},
		},
		nil, nil,
	)
...
func TestGetCampaignFunnel_InvalidTimeRange(t *testing.T) {
	svc := newTestService(nil, nil, &stubCampaignQueryRepo{}, nil, nil)
...
func (s *stubDeliverabilityQueryRepo) ListDeliverability(ctx context.Context, workspaceID string, filter domain.DeliverabilityFilter) ([]domain.DeliverabilityProjection, error) {
...
func TestSearchEvents_ValidationError(t *testing.T) {
	svc := newTestServiceWithForensicQuery(nil, &stubForensicQueryRepo{}, nil)
...
func TestGetOutboxLag_InvalidTimeRange(t *testing.T) {
	svc := newTestServiceWithOperationsQuery(nil, nil, nil)
...
func TestGetUsageTimeSeries_InvalidInterval(t *testing.T) {
	svc := newTestServiceWithUsageQuery(nil, nil, nil)
