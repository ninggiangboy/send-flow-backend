...
func (h *Handler) Execute(ctx context.Context, input Input) (*Result, error) {
	...
	log := h.log.With("provider", provider)

	if provider == "" {
		return nil, domain.ErrProviderNotSupported
	}

	verifier, ok := h.providerRegistry.Verifier(provider)
	if !ok {
		log.Warn("unsupported provider")
		return nil, domain.ErrProviderNotSupported
	}
	normalizer, ok := h.providerRegistry.Normalizer(provider)
	if !ok {
	...
	if err := h.txManager.WithinTx(ctx, func(txCtx context.Context) error {
		...
		if normErr == nil && normalized.ProviderMessageID != "" {
			ref, resolveErr := h.messageResolver.FindByProviderMessageID(txCtx, provider, normalized.ProviderMessageID)
		...
		if err := h.rawEventsWrite.Create(txCtx, rawEvent); err != nil {
			if errors.Is(err, domain.ErrDuplicateEventConflict) {
				existing, findErr := h.rawEventsRead.FindByProviderEventID(txCtx, provider, rawEvent.ProviderEventID)
		...
		if normErr == nil && normalized != nil {
			...
			if err := h.normalizedEventsWrite.Create(txCtx, normEvent); err != nil {
				if errors.Is(err, domain.ErrDuplicateEventConflict) {
					existing, findErr := h.normalizedEventsRead.FindByProviderEventID(txCtx, provider, normalized.ProviderEventID, normalized.EventType)
					if findErr == nil && existing != nil {
