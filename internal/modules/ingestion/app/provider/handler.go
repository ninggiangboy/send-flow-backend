...
func New(opts Options) *Handler {
	...
	return &Handler{
		...
		idGen:                 opts.IDGen,
		log:                   opts.Logger.With("usecase", "ingest_provider_webhook"),
	}
}
...
func (h *Handler) Execute(ctx context.Context, input Input) (*Result, error) {
	...
	log := h.log.With("provider", providerName)

	if providerName == "" {
		return nil, domain.ErrProviderNotSupported
	}

	verifier, ok := h.providerRegistry.Verifier(providerName)
	if !ok {
		log.Warn("unsupported provider")
		return nil, domain.ErrProviderNotSupported
	}
	normalizer, ok := h.providerRegistry.Normalizer(providerName)
	if !ok {
	...
	if err := h.txManager.WithinTx(ctx, func(txCtx context.Context) error {
		...
		var idErr error
		rawEventID, idErr = h.idGen()
		if idErr != nil {
			return idErr
		}
		if normErr == nil && normalized.ProviderMessageID != "" {
			ref, resolveErr := h.messageResolver.FindByProviderMessageID(txCtx, providerName, normalized.ProviderMessageID)
		...
		if err := h.rawEventsWrite.Create(txCtx, rawEvent); err != nil {
			if errors.Is(err, domain.ErrDuplicateEventConflict) {
				existing, findErr := h.rawEventsRead.FindByProviderEventID(txCtx, providerName, rawEvent.ProviderEventID)
		...
		if normErr == nil && normalized != nil {
			var nidErr error
			normalizedEventID, nidErr = h.idGen()
			if nidErr != nil {
				return nidErr
			}
			...
			if err := h.normalizedEventsWrite.Create(txCtx, normEvent); err != nil {
				if errors.Is(err, domain.ErrDuplicateEventConflict) {
					existing, findErr := h.normalizedEventsRead.FindByProviderEventID(txCtx, providerName, normalized.ProviderEventID, normalized.EventType)
					if findErr == nil && existing != nil {
			...
			normEnvelopeEventID, err := h.idGen()
			if err != nil {
				return err
			}
